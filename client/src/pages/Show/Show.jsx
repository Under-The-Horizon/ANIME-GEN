                ...
                <div className="u-non-blurred w-screen h-full overflow-x-hidden relative scrollbar-hide mt-16">
                    <div className="pl-8 mb-2 flex gap-16 static mt-[10%] flex-col flex-wrap overflow-x-scroll scrollbar-hide">
                        <div className=" w-[97%] h-max flex">
                            <div>
                                <div className="font-bold text-red-300 m-2">#1 Most Popular</div>
                                ...
                                <div className="m-2 flex gap-6">
                                    ...
                                    <div className="flex gap-1">
                                        <div className="bg-red-400 p-1 rounded-l-lg"><i className="ri-play-circle-fill"></i>{data?.infoX[1]?.statusAnime.toUpperCase()}</div>
                                        <div>
                                            {data?.infoX[1]?.premired ? (
                                                <div className="bg-red-400 p-1 rounded-r-lg"><i className="ri-calendar-fill"></i>{data?.infoX[1]?.premired}</div>
                                            ) : data?.infoX[1]?.aired ? (
                                                <div className="bg-red-400 p-1 rounded-r-lg"><i className="ri-calendar-fill"></i>{data?.infoX[1]?.aired}</div>
                                            ) : null}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="m-2 flex gap-[3px]">
                                    <div className="bg-red-400 p-[1px] text-xs rounded-l-lg">{data?.infoX[1]?.genre[0]}</div>
                                    {data?.infoX[1]?.genre.slice(1, -1).map((genre, index) => (
                                        <div key={genre} className="bg-red-400 p-[1px] text-xs">{genre}</div>
                                    ))}
                                    <div className="bg-red-400 p-[1px] text-xs rounded-r-lg">{data?.infoX[1]?.genre.slice(-1)}</div>
                                </div>
                                <div className="m-2 my-3 flex gap-2">
                                    <a href="#" className="flex p-2 px-3 text-white font-bold bg-red-600 rounded-lg hover:scale-105 group">
                                        <div> Watch Now </div>
                                        <i className="ri-play-large-fill ml-2 group-hover:scale-125"></i>
                                    </a>
                                    <a href={data?.infoX[2]?.animechar[0]?.length > 0 ? data?.infoX[2]?.animechar[0].source : '#'} className="flex p-2 px-3 text-white font-bold border border-red-600 rounded-lg hover:scale-105 group">
                                        <div> Trailer</div>
                                        <i className="ri-play-circle-line ml-2 group-hover:scale-110"></i>
                                    </a>
                                    <div className="flex p-2 px-3 text-white font-bold bg-red-600 rounded-lg hover:scale-105 group">
                                        <div>Wishlist</div>
